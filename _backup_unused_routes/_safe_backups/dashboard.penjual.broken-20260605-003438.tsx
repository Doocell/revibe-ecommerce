...
import {
  RoleGuard } from "@/components/RoleGuard";
import {
...
import {
  uploadProductImages } from "@/lib/product-image-upload";

const db = supabase as any;

export const Route = createFileRoute("/_safe_backups/dashboard/penjual/broken-20260605-003438")({
  component: () => (
...
function SellerDashboardPage() {
  ...
  async function loadDashboardData() {
    ...
    try {
      ...
      const { data: complaintRows, error: complaintError } = await db
      ...
      if (complaintError) {
        console.error("[Seller Dashboard Complaint Count Error]", complaintError);
        setActiveComplaintCount(0);
      } else {
        setActiveComplaintCount((complaintRows ?? []).length);
      }
    ...
    },
    {
      ...
      description:
        activeComplaintCount > 0
          ? `${activeComplaintCount} komplain aktif perlu dicek atau direspons.`
          : "Lihat dan respons komplain yang diajukan pembeli.",
      icon: AlertTriangle,
    },
    {
      key: "report",
